...
public interface QuoteRepository extends JpaRepository<Quote, Long> {
    ...
    /** 전체 목록(거래처 포함, 최신 등록순) */
    @Query("select q from Quote q left join fetch q.client order by q.id desc")
    List<Quote> findAllWithClient();

    /** 서비스 내용(title) 또는 고객명 부분일치 검색(대소문자 무시, 거래처 포함, 최신 등록순) */
    @Query("select q from Quote q left join fetch q.client"
            + " where lower(q.title) like lower(concat('%', :keyword, '%'))"
            + " or lower(q.customerName) like lower(concat('%', :keyword, '%')) order by q.id desc")
    List<Quote> search(@Param("keyword") String keyword);
